...
def parse_catalog(
    csv_content: str, cutoff_year: int, languages: list, logger: logging.Logger
) -> list:
    """
    ...
    for row in reader:
        ...
        # Language filtering
        if languages != ["all"]:
            if row_lang not in [lang.lower() for lang in languages]:
                stats["wrong_lang"] += 1
                continue
    ...
    logger.info("Catalog parsing complete:")
...
def clean_gutenberg_text(raw_text: str) -> str:
    ...
    text = re.sub(
        r"\[[A-Z]{1,3},\s*(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s*\d{4}:[^\]]*\]",
        "",
        text,
    )
...
def download_text(
    book: BookMetadata, session: requests.Session, output_dir: Path, logger: logging.Logger
) -> tuple:
    # Check for cancellation before starting new download
    if cancellation_event.is_set():
        return (book.id, False, "cancelled")
...
def print_interruption_summary(output_dir: Path, total_books: int, logger: logging.Logger):
    """Print summary when download is interrupted."""
    # Count downloaded files
    downloaded_count = 0
    for lang_dir in output_dir.iterdir():
        if lang_dir.is_dir():
            downloaded_count += len(list(lang_dir.glob("*.txt")))
    ...
    print(f"  Books downloaded this run: {downloaded_count:,}")
...
def collect_corpus(
    ...
    try:
        ...
        print_interruption_summary(output_dir, len(books), logger)
        sys.exit(0)
...
Examples:
  ...
  python gutenberg_collector.py -y 500 -l all -o ./corpus_ancient
    ...
    parser.add_argument(
        "--language", "-l", default="en", help="Language(s): 'en', 'en,fr,de', or 'all'"
    )
    ...
    if args.language.lower() == "all":
        ...
        languages = [lang.strip().lower() for lang in args.language.split(",")]
    ...
    collect_corpus(
        args.cutoff_year, languages, args.output_dir, args.concurrency, args.limit, logger
    )
